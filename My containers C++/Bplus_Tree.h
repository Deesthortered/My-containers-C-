...
#pragma comment(lib, "C:\\Users\\morga\\source\\repos\\2_course_2_sem\\algo_labs\\B+_Tree\\B+_Tree\\gvc.lib")
#pragma comment(lib, "C:\\Users\\morga\\source\\repos\\2_course_2_sem\\algo_labs\\B+_Tree\\B+_Tree\\cgraph.lib")
#pragma comment(lib, "C:\\Users\\morga\\source\\repos\\2_course_2_sem\\algo_labs\\B+_Tree\\B+_Tree\\cdt.lib")
...
///		int								print						(Node<T> *p, bool nextline);
	...
	public:
		...
		Node<T>							*search						(T &x, int *index = nullptr);
	...
	private:
		...
		bool convert_dot_to_png(string dot_filename, string png_filename);
...
//		delete k;
	...
	template<class T>
	Node<T>* BpTree<T>::search(T & x, int *index /* = nullptr */)
	{
		if (!root)
			return root;
		Node<T> *p = root;
		int pos = p->binSearch(x, false);
		while (p->leaf == false)
		{
			p = p->keys[pos];
			pos = p->binSearch(x, true);
		}
		if (index)
			*index = pos;
		return p;
	...
	template<class T>
	inline bool BpTree<T>::remove(T & x)
	{
		Node<T> *p = search(x);
		int pos = p->binSearch(x, false);
		if (p->data[pos] == x)
			p->erase(pos, true);
		else if (pos!=0 && p->data[pos - 1] == x)
			p->erase(pos, true);
		else return 0;
		--size;
		merge(p);
	...
	{
		std::string o_arg = std::string("> ") + png_filename;
	...
	//	string command = "dot -Tpng tmp.gv > " + fname;
		//system("cd bin");
